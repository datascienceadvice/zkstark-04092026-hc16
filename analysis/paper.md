...
- dev-матрица (последний полноценный прогон, run 13): 20/20, 19 `ok` +
  1 `expected_error` (константная группа, намеренная SW-ошибка), все
  проверки `pass`, total ~86.6M циклов за ~8 c.
- full (настоящие STARK-proof): n16, n02, n05, n20, n01 — все `pass`;
  композиция (`env::verify`) работает в боевом режиме (n01: 727 c,
  1.21M циклов). Медианный n06 в full упирается в память prover-сервера
